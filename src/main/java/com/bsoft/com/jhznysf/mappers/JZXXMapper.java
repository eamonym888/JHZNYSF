...
public interface JZXXMapper extends SqlMapper {
    ...
    @Select(" <script> SELECT A.*\n" +
            "FROM (\n" +
            "SELECT (SELECT GX.MRZ FROM GY_XTCS GX where GX.CSMC='YLJGDM') AS ORGAN_CODE,MG.BRID,MB.MZHM AS VISIT_CARD_NO,MB.BRXM,MB.SFZH,MB.BRXB,decode(MB.BRXB,1,'男','女') AS SEX_NAME, \n" +
