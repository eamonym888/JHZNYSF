...
public interface JZXXMapper extends SqlMapper {
    ...
    @Select(" <script> SELECT A.*\n" +
            "FROM (\n" +
            "SELECT GX.MRZ AS ORGAN_CODE,MG.BRID,MB.MZHM AS VISIT_CARD_NO,MB.BRXM,MB.SFZH,MB.BRXB,decode(MB.BRXB,1,'男','女') AS SEX_NAME, \n" +
            "MB.CSNY,MB.JTDH,MB.MZHM AS OUTHOSP_NO,YMJ.JZXH AS OUTHOSP_SERIAL_NO,MG.BRXZ,DECODE(MB.BRXZ,1000,'自费','医保') AS PAT_TYPE_NAME, \n" +
            "MG.SBXH AS REGIST_NO,to_char(MG.GHSJ,'YYYY-MM-DD hh:mm:ss') AS GHSJ,to_char(YMJ.KSSJ,'YYYY-MM-DD hh:mm:ss') AS KSSJ,to_char(YMJ.JSSJ,'YYYY-MM-DD hh:mm:ss') AS JSSJ,SBO.ZSXX AS CHIEF_DESCR,SBO.XBSXX AS CURR_DISEASE_HISTORY ,SBO.XJWS AS PAST_DISEASE_HISTORY, \n" +
            "'' AS ALLERGY_HISTORY, YMJ.YSDM,GY.YGXM AS RECEPT_TREAT_DR_NAME,YMJ.KSDM,GK.KSMC AS VISIT_DEPT_NAME,\n" +
            ...
            "INNER JOIN YS_MZ_JZLS YMJ ON YMJ.JZXH=MG.SBXH\n" +
            "LEFT JOIN GY_YGDM GY ON GY.YGDM=YMJ.YSDM\n" +
            "LEFT JOIN GY_KSDM GK ON GK.KSDM=YMJ.KSDM\n" +
